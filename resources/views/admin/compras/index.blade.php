        ...
        <div class="purchase-page">
                    ...
                    <div class="table-wrap">
                        <table class="data-table">
                            ...
                            @foreach($sugerencias as $index => $sugerencia)
                                @php($material = $sugerencia['material'])
                                <tr>
            ...
            <section class="section" id="solicitudes">
                ...
                <div class="request-list">
                    @forelse($solicitudes as $solicitud)
                        <article class="request-card" id="solicitud-{{ $solicitud->id }}" data-state="{{ $solicitud->estado }}">
                            ...
                            @elseif($isAdmin && $solicitud->estado === 'autorizada')
                                <form class="request-actions" method="POST" action="{{ route('admin.compras.requests.order', $solicitud) }}">
                                    @csrf
                                    <div class="request-actions two">
                                        <label class="muted">Proveedor<input name="proveedor" required placeholder="Proveedor elegido"></label>
                                    ...
                                    @foreach($solicitud->items as $item)
                                        <label class="muted">Precio de {{ $item->material?->descripcion }}
                                            <input type="number" name="costo_unitario[{{ $item->id }}]" min="0" step="0.01" value="{{ $item->material?->costo_unitario ?? 0 }}">
                                        </label>
                                    @endforeach
                    ...
                    @empty
                        <div class="empty">Aun no hay solicitudes de compra.</div>
                    @endforelse
                </div>
